# Merged create_update_backup_delete.py - PREAMBLE
# This preamble takes a snapshot of seriesData.json BEFORE the main logic runs,
# so we can label new entries as "First Time Created" in post-processing.
import os, shutil, json, time, datetime, glob, re
__MERGED_START_TS = time.time()
__PRE_SNAPSHOT_FILE = "seriesData_pre_run.json"
try:
    if os.path.exists("seriesData.json"):
        shutil.copyfile("seriesData.json", __PRE_SNAPSHOT_FILE)
    else:
        with open(__PRE_SNAPSHOT_FILE, "w", encoding="utf-8") as __pf:
            json.dump([], __pf)
except Exception as __e:
    print("Warning: failed to take pre-run snapshot:", __e)
# load pre-run titles
__PRE_TITLES = set()
try:
    with open(__PRE_SNAPSHOT_FILE, "r", encoding="utf-8") as __pf:
        __pdata = json.load(__pf)
        if isinstance(__pdata, list):
            for __o in __pdata:
                __t = __o.get("showName") or __o.get("title") or __o.get("name")
                if __t:
                    __PRE_TITLES.add(str(__t).strip().lower())
except Exception:
    __PRE_TITLES = set()
# Record start time for report file discovery
__MERGED_START_TIME = datetime.datetime.now()
# End preamble
...
# Merged create_update_backup_delete.py - POST-PROCESSING APPENDIX
# This appendix finds the report produced by the original script,
# parses it, normalizes sections (dedupe, "First Time Created" labels),
# hides empty sheets, and writes a cleaned report reports/report_<ts>_fixed.txt
import os, re, glob, time, datetime, json
def __find_original_report(start_time):
    # prefer any file that was modified after start_time
    files = sorted(glob.glob('reports/report_*.txt'), key=os.path.getmtime, reverse=True)
    for p in files:
        try:
            ...
            continue
    # fallback: latest reports file
    return files[0] if files else None

def __parse_report(path):
    ...
    for ln in lines:
        s = ln.strip()
        # header like === Sheet1 — DATE ===
        if re.match(r'^={3,}\s*.+?\s*—', s):
            parts = s.split('—', 1)
            name = parts[0].strip('= ').strip()
            current = name
            parsed[current] = {'created': [], 'updated': [], 'skipped': []}
            section = None
            continue
        if s.startswith('Data Created:'):
            section = 'created'; continue
        if s.startswith('Data Updated:'):
            section = 'updated'; continue
        if s.startswith('No Modification') or s.startswith('No Modification (Skipped)') or s.startswith('No Modification, Skipped'):
            section = 'skipped'; continue
        if section and s.startswith('-'):
            title = s[1:].split('->',1)[0].strip()
            if title and current:
                parsed[current][section].append(title)
    return parsed, lines

def __build_fixed(parsed, pre_titles):
    parts = []
    parts.append('✅ Workflow completed successfully')
    parts.append('📅 Run Time: ' + datetime.datetime.now().strftime('%d %B %Y %I:%M %p (IST)'))
    parts.append('')
    overall = {'created': set(), 'updated': set(), 'skipped': set(), 'images_updated': 0, 'warnings': 0, 'failed': 0}
    for sheet, data in parsed.items():
        created = list(dict.fromkeys(data.get('created', [])))
        updated = list(dict.fromkeys(data.get('updated', [])))
        skipped = list(dict.fromkeys(data.get('skipped', [])))
        if not (created or updated or skipped):
            continue
        parts.append(f"=== {sheet} — {datetime.datetime.now().strftime('%d %B %Y')} ===")
        parts.append('')
        # Data Created
        if created:
            parts.append('Data Created:')
            for t in created:
                key = t.strip().lower()
                label = 'First Time Created' if key not in pre_titles else 'Created'
                parts.append(f"- {t} -> {label}")
                overall['created'].add(t)
        else:
            parts.append('Data Created:'); parts.append('- None')
        parts.append('')
        # Data Updated
        if updated:
            parts.append('Data Updated:')
            for t in updated:
                parts.append(f"- {t} -> Updated")
                overall['updated'].add(t)
        else:
            parts.append('Data Updated:'); parts.append('- None')
        parts.append('')
        # No Modification (Skipped)
        if skipped:
            parts.append('No Modification (Skipped):')
            for t in skipped:
                parts.append(f"- {t}")
                overall['skipped'].add(t)
        else:
            parts.append('No Modification (Skipped):'); parts.append('- None')
        parts.append('')
        total_rows = len(set(created + updated + skipped))
        parts.append('Summary (Sheet: %s)' % sheet)
        parts.append('-' * 40)
        parts.append(f"Created: {len(created)}")
        parts.append(f"Updated: {len(updated)}")
        parts.append(f"Skipped: {len(skipped)}")
        parts.append(f"Images Updated: {overall['images_updated']}")
        parts.append(f"Warnings: {overall['warnings']}")
        parts.append(f"Failed: {overall['failed']}")
    ...
    parts.append('-' * 40)
    parts.append(f"Created: {len(overall['created'])}")
    parts.append(f"Updated: {len(overall['updated'])}")
    parts.append(f"Skipped: {len(overall['skipped'])}")
    parts.append(f"Images Updated: {overall['images_updated']}")
    parts.append(f"Warnings: {overall['warnings']}")
    parts.append(f"Failed: {overall['failed']}")
    # backups count
    ...
    try:
        ...
        backups = 0
    parts.append(f"Backup files: {backups}")
    parts.append('')
    parts.append('🏁 Workflow finished successfully')
    return '\n'.join(parts)

def __write_fixed(text):
    os.makedirs('reports', exist_ok=True)
    out = f"reports/report_{datetime.datetime.now().strftime('%d_%B_%Y_%H%M')}_fixed.txt"
    with open(out, 'w', encoding='utf-8') as f:
        f.write(text)
    print(f"Report written -> {out}")
    return out

# run post-processing now
try:
    __orig_report = __find_original_report(__MERGED_START_TIME)
    __parsed, __raw = __parse_report(__orig_report)
    __final = __build_fixed(__parsed, __PRE_TITLES)
    __fixed_path = __write_fixed(__final)
    # print markers
    print('\n===FIXED_REPORT_START===')
    print(__final)
    print('===FIXED_REPORT_END===\n')
except Exception as e:
