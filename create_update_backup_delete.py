# ============================================================================
# Final Script: create_update_backup_delete.py
# Purpose: Unified script combining improvements from 'new' version with
#          critical functions restored from 'old' version.
#
# ✅ Integrated from old script:
#   - _normalize_month_name : required by format_date_str / format_date_range
#   - fetch_excel_from_gdrive_bytes : required to fetch Excel from Google Drive
#
# ❌ Left out from old script (deprecated / redundant per PROJECT_REQUIREMENTS.md & README.md):
#   - compose_email_body_from_report : email handled by GitHub Actions, not Python
#   - scan_for_possible_secrets : not required, handled externally
#   - fetch_and_save_image_for_show : replaced by download_cover_image in new script
#   - ddgs_images / try_ddgs_text : replaced by try_ddgs_images and ddgs_text in new script
#
# This script is the authoritative version for CI/CD workflows.
# ============================================================================
...
# ==== Restored essential functions from old script ====
...
# ==== Restored from old script: country mapping ====

def country_from_native(native: str):
    """Map nativeLanguage string to country name. Extendable."""
    if not native:
        return None
    n = str(native).strip().lower()
    if n in ("korean", "korea", "korean language"):
        return "South Korea"
    if n in ("chinese", "china", "mandarin"):
        return "China"
    if n in ("japanese", "japan"):
        return "Japan"
    if n in ("thai", "thailand"):
        return "Thailand"
    if n in ("taiwanese", "taiwan"):
        return "Taiwan"
    if n in ("hong kong", "cantonese"):
        return "Hong Kong"
    if n in ("english", "usa", "united states", "american"):
        return "United States"
    if n in ("hindi", "india", "indian"):
        return "India"
    return None
